...
From what I can tell, there is much interest in the recent [Wasserstein-GAN paper](https://arxiv.org/abs/1701.07875). It, or at least [this excellent summary](http://www.alexirpan.com/2017/02/22/wasserstein-gan.html), is definitely worth reading. In this post, I don't want to repeat the mechanics and promised benefit of WGANs. Instead, we will focus mainly on one detail that is only mentioned quickly, but I think lies in some sense at the heart of it: the Kantorovich-Rubinstein duality, or rather a special case of it. This is of course not a new result, but the application is very clever.

The paper cites the book "Optimal Transport - Old and New" by Fields-Medal winner and french eccentric Cedric Villani, you can [download it from his homepage](cedricvillani.org/wp-content/uploads/2012/08/preprint-1.pdf) (large pdf file!). That's about a thousand pages targeted at math PHDs and researchers - have fun! Villani talks about this topic in an accessible way in [this lecture](https://www.youtube.com/watch?v=zo46TEp6FB8), at around the 28 minute mark. Generally though, I found it very hard to find material that gives real explanations but is not bursting with definitions and references to theorems I didn't know. Maybe this post will help to fill this gap little bit. We will only use basic linear algebra, probability theory and optimization. These will not be rigorous proofs, and we will generously imply many regularity conditions. But I tried to make the chains of reasoning as complete as possible, and it should be enough get some intuition for this subject.

The argument for our case of the Kantorovich-Rubinstein duality is actually not too complicated and stands for itself. It is, however, very abstract, which is why I decided to defer it to the end and start with the discrete case and somewhat related problems in Linear Programming.
...
Lastly, we have to consider continuous probability distributions.
...
- [Zemel: Optimal Transportation: Continuous and Discrete](smat.epfl.ch/~zemel/vt/pdm.pdf)
---
...
This means of course that, if  $\sup_{\ b \in B} g(a,b)$ is convex and $\inf_{\ a \in A} g(a,b)$ is concave, then the minimax principle applies and $g(\hat{a}, \hat{b}) = g(\hat{a}', \hat{b}')$.

In our case, we can already see from the underbrace, we already see that the convexity condition is met. Let's try changing to $\sup \inf$:
